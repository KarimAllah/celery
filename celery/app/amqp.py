...
class MultiPublisherPool(ProducerPool):
    def __init__(self, app, next=False):
        self.app = app
        super(MultiPublisherPool, self).__init__(self.app.get_pool(next),
                                            limit=self.app.pool.limit)
...
class AMQP(object):
    ...
    _multipublisher_pool = None
    def get_publisher_pool(self, next=False):
        if self._multipublisher_pool is None or next:
            self._multipublisher_pool = MultiPublisherPool(self.app, next)

        return self._multipublisher_pool
