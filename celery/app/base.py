...
class BaseApp(object):
    ...
    def broker_connection(self, hostname=None, userid=None,
            ...
            transport_options=None, next_connection= False, **kwargs):
        ...
        """
        conf = self.conf
        hostnames = (hostname or conf.BROKER_HOST).split(';')
        
        if hostname or (self.connection_index == len(hostnames) - 1):
            self.connection_index =  0
        else:
            if next_connection:
                self.connection_index += 1
            
        hostname = hostnames[self.connection_index]
